...
$(document).ready(function () {
  ...
  wishlistButtons.forEach(button => {
    button.addEventListener('click', function (e) {
      ...
      e.preventDefault();    // Prevent default anchor behavior (navigating to product page)
      e.stopPropagation();   // Stop the event from bubbling up

      const handle = this.dataset.productHandle;

      // Toggle the active class for styling purposes (change color or appearance)
      this.classList.toggle('active');

      // Example: Save to localStorage (this could be your "wishlist" logic)
...
if (window.innerWidth < 768) {
  document.querySelectorAll('.product-metafield-ingredients').forEach(el => {
    const maxLength = 55;
